...
const GET_URI = "/api/v1/read/page/ACOB-VPAA-ASC-HALSTORE/"
...
async function completeTask(t) {
  ...
  if (person.roles.includes(ROLE) && givenContent.length > 0) {
    personPage = await getAsset(person.pagePath);

    // console.log("person prep done: " + person.uri);
    // console.log("person page");
    // console.dir(personPage);
      
    // preparePayload
    // enumerateSections(person);
    var payload = preparePayload(person, personPage);
...
function prepareContent(content) {
  ...
  sections.map(function(s) {
    ...
    if (!(headingNode === undefined)) {
      ...
      // console.log("\t parsed section: " + lookup);
      headingNode.extract();
...
function preparePayload(person, personPage) {
  // console.log("*** preparePayload ***");
  var asset = personPage;
  ...
  asset.asset.page.structuredData.structuredDataNodes.map(function(sdn) {
    ...
    if ((sdn.identifier == 'fullBio') || (sdn.identifier == 'teaching') || (sdn.identifier == 'researchInterests') || (sdn.identifier == 'degrees') || (sdn.identifier == 'publications')) {
      ...
      if (contentAreas[sdn.identifier]) {
        // console.log("pushing: " + sdn.identifier);
...
function cleanContent(content) {
  ...
  contentStr = contentStr.replaceAll('”', '&#8221;');
  contentStr = contentStr.replaceAll('\&quot;', '');
  contentStr = contentStr.replaceAll('”', '&#3221;');
  return contentStr;
}
