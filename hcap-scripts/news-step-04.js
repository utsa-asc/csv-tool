/* POST Process Tasks:
    // our only goal here is to use the CSV data and snippet content to craft 
    // our POST payload and successfully create a news post on Cascade via the REST API
  0) read incoming CSV
  1) read snippet html from local disk
  2) craft REST API POST payload
  3) POST to Cascade API
  4) PROFIT!
  */
  ...
  fs.createReadStream('hcap/hcap-news-2022-step-01.csv')
    ...
    */
    ...
    /* compute the source URL for the original media uploaded to WordPress */
    ...
    /* given a URL, download the content to disk, not much error checking going on here (BAD) */
