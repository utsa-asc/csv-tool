    ...
    const get = protocol.request(getOptions, res => {
      ...
      res.on('end', function() {
        ...
        if (responseObj.asset) {
          ...
          var detailsSDNs = responseObj.asset.xhtmlDataDefinitionBlock.structuredData.structuredDataNodes;
          console.log(detailsSDNs);
          var newLinkAlt = {
            "type": "text",
            "identifier": "label",
            "text": displayName
          ...
          var groupLink = {};
          detailsSDNs.map(function(item) {
            if (item.identifier == "image") {
              item.structuredDataNodes.map(function(linkItem) {
                if (linkItem.identifier == "alt") {
                  linkItem.text = displayName;
                }
              });
            }
          });
