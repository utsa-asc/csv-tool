...
function updatePaths(t) {
  ...
  //update spreadsheet
  //update image cell
  XLSX.utils.sheet_add_aoa(dataSheet, [[t.image]], { origin: kOrigin });
  //update parentFolderPath cell
  XLSX.utils.sheet_add_aoa(dataSheet, [[t.parentFolderPath]], {
    origin: nOrigin,
  });
  //update uri cell
  XLSX.utils.sheet_add_aoa(dataSheet, [[t.uri]], { origin: eOrigin });
...
function patchMonth(soupSnippet, task, sheet) {
  ...
  XLSX.utils.sheet_add_aoa(sheet, [[task.month]], { origin: cOrigin });
  return soupSnippet;
}
...
function saveSnippet(content, task) {
  ...
  XLSX.utils.sheet_add_aoa(dataSheet, [[fpath]], { origin: oOrigin });
